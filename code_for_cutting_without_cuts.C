...
#include "mpdloadlibs.C"
 ...
 TH2D *hdEdxP           = new TH2D("hdEdxP","Plot for dE/dx and total momentum(P)",100,-2,2,100,0,10);
 ...
 TH1F *hRecTrack_P 	    = new TH1F("hRecTrack_P","Charged tracks total momentum(P) Distribution (Reco)",3200,0,2.5); 
 TH1F *hRecTrack_gpT    = new TH1F("hRecTrack_gpT","Charged global tracks total momentum(GP) Distribution (Reco)",3200,0,2.5); 
 ...
 TH2D *hetapt           = new TH2D("hetapt","plot of rapidity(y) and transverse momentum(pT)", 500,-2.5,2.5,60,0,2);
 TH1F *hetapt1F         = new TH1F("hetapt1F","1D-plot of rapidity(eta) and transverse momentum(pT)",500,-2.5,2.5);   
 TH2D *hphiVTXz         = new TH2D("hphiVTXz","Plot for vertexZ and polar angle(phi)",500,-160,160,500,-4,4);   

 //_______________________________________________________________________________________
 void trial_without(const Char_t* inFileName, int myevents, TString outFileName ) {
  ...
  for (Long64_t i = 0; i < myevents; i++) {
   ...
   // Float_t bField = event->bField();
    ...
    Float_t b               = mcEvent->b(); // Impace parameter From MC
   ...
   //==================================================//
    ...
    for (Int_t j = 0; j < nRecoTracks; j++) {
     ...
     // Float_t VTXxy = TMath::Sqrt(vertexX*vertexY + vertexX*vertexY);
     ...
     //  TVector3 mpdTrack_gtDCA  = miniTrack->gDCA();
      ...
      hRecTrack_gpT->Fill(mpdTrack_gpT);
  ...
  hRecTrack_pT->GetXaxis()->SetTitle("Transverse momentum P with primary hits");
  hRecTrack_pT->GetXaxis()->CenterTitle(true);
  hRecTrack_pT->Write();
   
  hRecTrack_gpT->GetXaxis()->SetTitle("Transverse momentum P with global hits");
  hRecTrack_gpT->GetXaxis()->CenterTitle(true);
  hRecTrack_gpT->Write();
